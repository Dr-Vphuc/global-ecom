...
\textit{Interactive Data Visualization: Foundations, Techniques, and Applications} của Matthew O. Ward, Georges Grinstein và Daniel Keim là một giáo trình nền tảng của lĩnh vực trực quan hóa dữ liệu, với đặc điểm là mỗi kỹ thuật đều được trình bày kèm mã giả và những đánh đổi khi cài đặt. Chúng tôi chọn dịch Chương 9 --- \textit{Visualization Techniques for Trees, Graphs, and Networks} --- vì cây và đồ thị là hai cấu trúc quan hệ phổ biến nhất trong thực tế, và cũng vì đây chính là họ kỹ thuật nhóm sử dụng cho đồ án cuối kỳ của môn học.

Trong quá trình chuyển ngữ, chúng tôi đặt tính chính xác về mặt kỹ thuật lên trước sự mượt mà của câu văn. Những thuật ngữ đã thành tên riêng quốc tế như \textit{treemap}, \textit{sunburst}, \textit{node-link}, \textit{force-directed} hay \textit{cone tree} đều được giữ nguyên, vì người đọc sẽ gặp lại chúng nguyên văn trong tài liệu chuyên ngành; riêng \textit{node} được giữ nguyên để tránh nhầm với nghĩa thông thường của ``nút'' trong tiếng Việt, còn \textit{edge} dịch thống nhất là ``cạnh''. Các khối mã giả giữ nguyên tiếng Anh vì là mã lệnh chứ không phải văn xuôi, chỉ chú thích hình được dịch; hình minh họa và số hiệu tài liệu tham khảo cũng giữ theo bản gốc để người đọc đối chiếu được với nguyên tác. Riêng các chú thích chân trang là phần bổ sung của người dịch, không có trong nguyên tác, được thêm ở những chỗ bản gốc nêu kết luận nhưng lược bỏ lập luận dẫn tới kết luận đó. Đây là bản dịch của những người còn đang học nên khó tránh thiếu sót; chúng tôi rất mong nhận được góp ý từ thầy cô và các bạn đọc.
...
Treemap \cite{johnson1991} cùng vô số biến thể của nó là một cách biểu diễn thay thế cho biểu đồ Venn, và là dạng phổ biến nhất của bố cục chữ nhật lấp đầy không gian. Trong treemap cơ bản, một hình chữ nhật được chia đệ quy thành các lát, luân phiên cắt theo phương ngang và phương dọc, dựa trên số lượng phần tử của các cây con ở tầng đang xét. Mã giả của quá trình này được cho ở Hình \ref{fig:treemap_code}, và một ví dụ được trình bày ở Hình \ref{fig:treemap}.

Như đã đề cập, nhiều biến thể của treemap đã được đề xuất và phát triển kể từ khi kỹ thuật này ra đời, trong đó có treemap vuông hóa \cite{bruls2000} (tối ưu hóa việc phân chia sao cho tỉ lệ khung hình của mỗi ô gần với hình vuông nhất có thể)\footnote{Thuật toán mặc định chia lát cắt luân phiên theo một chiều cố định cho mỗi tầng, điều này tạo ra các ô rất dài và hẹp khi có nhiều node lá, gây khó khăn cho việc tương tác chọn lựa bằng chuột, ước lượng diện tích bằng mắt và hiển thị nhãn văn bản.} và treemap lồng nhau \cite{johnson1991} (nhằm làm nổi bật cấu trúc phân cấp).
...
Một khi đồ thị đã được xác định là phẳng, hoặc đã được bổ sung để đạt được tính phẳng, có rất nhiều chiến lược khả dĩ để sinh ra một bản vẽ. Một kỹ thuật như vậy, gọi là phương pháp visibility \cite{dibattista1998}, gồm một quy trình hai bước.
...
Renoust \cite{renoust2014} mô tả một cách phân tích mạng đa lớp. Ông định nghĩa một độ đo về mức độ quấn bện vào nhau của các cạnh (chỉ số đan bện), từ đó dẫn tới các độ đo khác (ví dụ: cường độ, và tính đồng nhất) và cuối cùng dẫn tới việc hiểu được mạng tương tác chất xúc tác.

Mạng thì rất đa dạng. Mạng phức tạp đã được nghiên cứu từ lâu, và việc mô tả đặc trưng cũng như định nghĩa chúng đến nay vẫn còn là chủ đề của nhiều công bố \cite{costa2007,kim2008}. Tuy vậy, ta có thể thừa nhận rằng mạng phức tạp là mạng thể hiện những đặc trưng mà ta không tìm thấy ở mạng đơn giản cũng như ở mạng ngẫu nhiên, nhưng lại rất thường gặp ở các mạng trong thế giới thực \cite{watts1998}. Cụ thể hơn về mặt thực nghiệm, các tài liệu đã phát hiện \cite{albert2002} và vẫn đang tiếp tục phát hiện \cite{pagani2013} những đặc trưng của mạng phức tạp trong hầu hết các mạng thực tế. Trong số các đặc trưng đó, nổi tiếng nhất là \cite{albert2002}:
...
Một mạng phức tạp thể hiện ít nhất một vài trong số các đặc trưng trên và các đặc trưng ấy không chỉ giới hạn ở những gì vừa nêu \cite{claussen2008}. Trong một đồ thị thực tế, ta cũng có thể quan sát thấy nhiều thành phần không liên thông với kích thước biến thiên rất mạnh (nhiều khả năng là do một đặc tính scale-free bên trong như mô tả ở trên gây ra). Một hoặc vài thành phần lớn nhất có thể thể hiện phần lớn các đặc trưng của mạng phức tạp.

Strogatz, trong công trình của mình \cite{strogatz2001}, định nghĩa sáu cách khác nhau để một mạng bộc lộ tính phức tạp:
...
\begin{figure}[htbp]
    ...
    (Hình ảnh từ \cite{yang2003}, \copyright~2003 Palgrave Macmillan.)}
    \label{fig:9.12}
\end{figure}
...
Robertson và cộng sự \cite{robertson1991} và Brian Johnson cùng Ben Shneiderman \cite{johnson1991} lần lượt giới thiệu các khái niệm về cây hình nón và treemap. John Stasko và Eugene Zhang \cite{stasko2000} mô tả một trong số các biến thể của các kỹ thuật lấp đầy không gian theo hướng xuyên tâm dùng cho trực quan hóa cây. Cuốn sách Graph Drawing: Algorithms for the Visualization of Graphs \cite{dibattista1998} là một tài liệu nhập môn xuất sắc về lĩnh vực vẽ đồ thị.The Semiology of Graphs \cite{bertin1983} của J. Bertin là công trình nền tảng về các biểu diễn ma trận có thể sắp xếp lại cho đồ thị. Herman và cộng sự \cite{herman2000} trình bày một khảo sát về trực quan hóa đồ thị và các hình thức tương tác với đồ thị. Bài báo của Leung và Apperley \cite{leung1994} cung cấp một khảo sát toàn diện về các kỹ thuật biến dạng, trong đó nhiều kỹ thuật có thể được áp dụng cho trực quan hóa cây và đồ thị. Renoust \cite{renoust2014} đề cập đến vấn đề trực quan hóa nhiều đồ thị. Các VAST Challenges cung cấp các bộ dữ liệu tổng hợp có thể được sử dụng làm các tình huống nghiên cứu điển hình.


% =====================================================================
% TÀI LIỆU THAM KHẢO
%
% Số hiệu trong ngoặc vuông được GIỮ NGUYÊN theo danh mục của sách gốc,
% không đánh số lại từ 1. Nhờ vậy người đọc tra cứu được trực tiếp sang
% nguyên tác. Nhãn hiển thị được ép bằng tham số tùy chọn của \bibitem.
%
% Trong phần nội dung, trích dẫn được viết bằng \cite{khóa} thay vì gõ
% tay số hiệu, để LaTeX tự cảnh báo khi có tài liệu bị trích dẫn nhưng
% chưa khai báo, hoặc khai báo nhưng không ai dùng.
% =====================================================================
...
\bibitem[7]{albert2002}
Réka Albert and Albert-László Barabási. “Statistical Mechanics of Complex Networks.” \textit{Reviews of Modern Physics} 74:1 (2002), 47.

\bibitem[27]{dibattista1998}
G. Di Battista, P. Eades, R. Tamassia, and I. G. Tollis. \textit{Graph Drawing: Algorithms for the Visualization of Graphs}. Upper Saddle River, NJ: Prentice Hall, 1998.

\bibitem[34]{bertin1983}
Jacques Bertin. \textit{Semiology of Graphics: Diagrams, Networks, Maps}. Madison, WI: University of Wisconsin Press, 1983. Translated by Wiliam J. Berg. Originally published in 1967 in French.

\bibitem[48]{brodbeck1997}
Dominique Brodbeck, Matthew Chalmers, Aran Lunzer, and Pamela Cotture. “Domesticating Bead: Adapting an Information Visualization System to a Financial Institution.” In \textit{Proceedings of the IEEE Symposium on Information Visualization '97}, pp. 73--80. Los Alamitos, CA: IEEE Computer Society Press, 1997.

\bibitem[51]{bruls2000}
M. Bruls, K. Huizing, and J. van Wijk. “Squarified Treemaps.” In \textit{Proceedings of Joint Eurographics and IEEE TCVG Symposium on Visualization}, pp. 33--42. Los Alamitos, CA: IEEE Computer Society Press, 2000. Available online (citeseer.ist.psu.edu/bruls99squarified.html).

\bibitem[78]{claussen2008}
J. C. Claussen and T. Wilhelm. “Introduction to Network Complexity.” In \textit{International Workshop and Conference on Complex Networks and Their Applications}. Norwich, UK: PUBLISHER, 2008.

\bibitem[85]{costa2007}
L. F. Costa, Francisco A. Rodrigues, Gonzalo Travieso, and Paulino Ribeiro Villas Boas. “Characterization of Complex Networks: A Survey of Measurements.” \textit{Advances in Physics} 56:1 (2007), 167--242.

\bibitem[186]{herman2000}
Ivan Herman, Guy Melançon, and M. Scott Marshall. “Graph Visualization and Navigation in Information Visualization: A Survey.” \textit{IEEE Transactions on Visualization and Computer Graphics} 6:1 (2000), 24--43. Available online (citeseer.ist.psu.edu/herman00graph.html).

\bibitem[206]{johnson1991}
Brian Johnson and Ben Shneiderman. “Tree-Maps: A Space-Filling Approach to the Visualization of Hierarchical Information Structures.” In \textit{VIS '91: Proceedings of the 2nd Conference on Visualization '91}, pp. 284--291. Los Alamitos, CA: IEEE Computer Society Press, 1991.

\bibitem[239]{kim2008}
Jongkwang Kim and Thomas Wilhelm. “What Is a Complex Graph?” \textit{Physica A: Statistical Mechanics and its Applications} 387:11 (2008), 2637--2652.

\bibitem[268]{leung1994}
Y. K. Leung and M. D. Apperley. “A Review and Taxonomy of Distortion-Oriented Presentation Techniques.” \textit{ACM Transactions on Computer-Human Interaction} 1:2 (1994), 126--160.

\bibitem[311]{pagani2013}
Giuliano Andrea Pagani and Marco Aiello. “Cost and Benefits of Denser Topologies for the Smart Grid.” In \textit{Computer and Information Sciences III}, edited by Erol Gelenbe and Ricardo Lent, pp. 73--81. London: Springer, 2013.

\bibitem[332]{renoust2014}
Benjamin Renoust. “Analysis and Visualisation of Edge Entanglement in Multiplex Networks.” Ph.D. thesis, University of Massachusetts Lowell, 2014.

\bibitem[341]{robertson1991}
George G. Robertson, Jock D. Mackinlay, and Stuart K. Card. “Cone Trees: Animated 3D Visualizations of Hierarchical Information.” In \textit{Proceedings of the SIGCHI Conference on Human Factors in Computing Systems}, pp. 189--194. New York: ACM Press, 1991.

\bibitem[393]{stasko2000}
John Stasko and Eugene Zhang. “Focus+Context Display and Navigation Techniques for Enhancing Radial, Space-Filling Hierarchy Visualizations.” In \textit{Proceedings of the IEEE Symposium on Information Visualization}, pp. 57--65. Los Alamitos, CA: IEEE Computer Society, 2000.

\bibitem[401]{strogatz2001}
Steven H. Strogatz. “Exploring Complex Networks.” \textit{Nature} 410:6825 (2001), 268--276.

\bibitem[452]{watts1998}
Duncan J. Watts and Steven H. Strogatz. “Collective Dynamics of ‘Small-World’ Networks.” \textit{Nature} 393:6684 (1998), 440--442.

\bibitem[481]{yang2003}
Jing Yang, Matthew O. Ward, Elke A. Rundensteiner, and Anilkumar Patro. “InterRing: A Visual Interface for Navigating and Manipulating Hierarchies.” \textit{Information Visualization} 2:1 (2003), 16--30.
